...
export const createNodeOfType = (type) => {
	return {
		type: type || '',
		name: '',
		value: '',
...
export const addChildNode = (ast, type) => {
	let len = 0;
	let arr;
...
export const assignContext = (ast, value) => {
	ast.current.context = value || '*' ;
};

export const addParameter = (ast, name, value) => {
...
export const unwireParent = (ast) => {
	ast.parents.pop();
	ast.current = ast.parents.length ? ast.parents[ast.parents.length-1] : ast;
};

export const quoteDelimiter = (str, idx, ast, buf, context) => {
...
export const whitespaceDelimiter = (str, idx, ast, buf, context) => {
	let chr = str[idx];
...
export const bangDelimiter = (str, idx, ast, buf, context) => {
...
export const hyphenDelimiter = (str, idx, ast, buf, context) => {
...
export const equalDelimiter = (str, idx, ast, buf, context) => {
...
export const slashDelimiter = (str, idx, ast, buf, context) => {
	if ( context & ( constants.CONTEXT_OPEN_COMMENT |
					 constants.CONTEXT_OPEN_PARAM_VALUE |
					 constants.CONTEXT_OPEN_TEXT ) ) {
		buf.push(str[idx]);
...
export const rightAngleDelimiter = (str, idx, ast, buf, context) => {
...
export const leftAngleDelimiter = (str, idx, ast, buf, context) => {
